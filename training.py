...
for intent in intents['intents']:
    for pattern in intent['patterns']:
        ...
        words_list.extend(
            tokenized_words)  # tokenize splits sentences into their component parts (hello I am happy becomes "hello" "I" "am" "happy"). these are then saved to the words list
...
for document in documents:
    ...
    word_patterns = [lemmatizer.lemmatize(word.lower()) for word in word_patterns]  # lowercases all words in a given document
...
model.add(tf.keras.layers.Dense(128, input_shape=(len(train_x[0]),), activation='relu'))    # Im aware this setup works but I struggle to explain the individual components
model.add(tf.keras.layers.Dropout(0.5))
model.add(tf.keras.layers.Dense(64, activation='relu'))
model.add(tf.keras.layers.Dropout(0.5))
model.add(tf.keras.layers.Dense(len(train_y[0]), activation='softmax')) # softmax scales % allocations of all outputs so that they total 100%

sgd = tf.keras.optimizers.SGD(learning_rate=0.01, momentum=0.9, nesterov=True)  # SGD stands for stochastic gradient descent and is an optimisation algorithm to find the best fit

model.compile(loss='categorical_crossentropy', optimizer=sgd, metrics=['accuracy'])
model.fit(train_x, train_y, epochs=200, batch_size=5, verbose=1)    # Epochs refer to how many times the data is run through the algorithm
